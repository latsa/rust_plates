...
use nanomsg_sys::{nn_socket, nn_bind, nn_connect, nn_poll, nn_send, nn_close, nn_errno, nn_strerror};

fn puller(url: &CStr) {

   unsafe {
      let input = nn_socket(nanomsg_sys::AF_SP as c_int, nanomsg_sys::NN_PULL);
      nn_bind (input, url.as_ptr());
      println!("Puller listening on '{}'.", url.to_str().unwrap());

      let mut pfd = nanomsg_sys::nn_pollfd {
         fd: input,
         events: nanomsg_sys::NN_POLLIN as i16 ,
         revents: nanomsg_sys::NN_ZERO as i16
      };
...
fn pusher(url: &CStr) {
   unsafe {
      let output = nn_socket(nanomsg_sys::AF_SP as c_int, nanomsg_sys::NN_PUSH);
